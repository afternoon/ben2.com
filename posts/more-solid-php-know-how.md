...
description: "Using the putenv and getenv functions UNIX enviroment variables can be used to store info on the server, but without tying it to any particular user (session). Maybe this can be used thus: getenv(\"PHP_WHOS_LOGGED_AT_HYPE\") would return a list of user id's active recently (maybe their times also as the names"
---
